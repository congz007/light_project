...
llp1$species[llp1$species =="Melastoma_candidum"] <-  'Melastoma candidum D. Don'
llp1$species[llp1$species =="Colocasia_gigantea"] <- 'Colocasia gigantea (Blume) Hook. f.'
llp1 |> 
  mutate(x_loc = Distance*cos(angel*pi/180))|>
  mutate(y_loc = Distance*sin(angel*pi/180))|>
  ggplot(aes(x_loc, y_loc, size = LMA,color = LMA))+

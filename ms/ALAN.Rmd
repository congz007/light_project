...
We evaluated whether ALAN would affect the LMA (leaf mass per area) of understory plants through a three-year field light experiment in tropical rubber plant forest, China.
...
We also predicted that canopy-openness and its interaction with artificial light at night would show weak impaction in this experiment.
...
A metal box with an opening is used as a rainproof protector which is attached to a tree at around 1.2m from the ground.
...
For leaf mass per area (LMA), we use leaf disc (10mm^2) punched from leaf avoiding vein and leaf margin instead of whole-leaf to calculate LMA value [@Maenpuen2022].

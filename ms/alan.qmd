...
We hypothesized that ALAN could impact the understory in two ways: by directly supplementing light to aboveground plant parts (which increases LMA) and indirectly affecting soil nutrient composition underground by attracting insects (which decreases LMA).
...
*Experimental setup* (**We need to add light intensity**)
...
We considered the understory conditions to select species, including numbers of mature individuals and distributions of individuals within each plot.
...
As a result, we chose two species, each from a different plot for our study:
*Colocasia gigantea*, representing shade species, and *Melastoma candidum* representing sun species.
...
On the other hand, ALAN did not have a significant effect on LMA in the sun species *M. candidum*.
Sun species receive stronger sunlight than shade species, which might result in the small effect of ALAN on their LMA.
Because the effect of daylight was not significant either, intraspecific variability (ITV) itself for sun species may inherently be smaller than for shade species.
...
](`r here::here("figs/lma_map.pngs")`){#fig-LMA}
